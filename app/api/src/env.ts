...
const schema = z
  .object({
    ...
    // Signing secret for the Stripe webhook. Without it the endpoint refuses
    // every delivery, because an unverified webhook can grant a paid plan to
    // anyone who can POST to a public URL.
    STRIPE_WEBHOOK_SECRET: z.string().optional(),
    // Price ids rather than plan definitions, so pointing at a different Stripe
...
export const env = {
  ...
  stripePriceStarter: parsed.data.STRIPE_PRICE_STARTER ?? null,
  stripePricePro: parsed.data.STRIPE_PRICE_PRO ?? null,
